    ...
    WHERE
        (
            ...
            OR (
                ol.status = 'Returned'
                AND EXISTS (
                    SELECT 1 FROM item_level il2
                    WHERE il2.order_id = ol.order_id AND il2.returned = 0
                )
            )
        )
        AND ol.order_date < date('now', '-90 days')
),
...
succ_order_values AS (
    ...
    FROM (
        ...
        WHERE (
            ol.status = 'Completed'
            OR (ol.status = 'Returned' AND EXISTS (
                SELECT 1 FROM item_level il2
                WHERE il2.order_id = ol.order_id AND il2.returned = 0
            ))
        )
...
return_cancel AS (
    ...
    FROM order_values ov JOIN succ_order_values sov USING(customer_id)
),

successful_orders_for_intervals_90d AS (
    SELECT ol.order_id, ol.customer_id, ol.order_date
    FROM order_level ol JOIN item_level il USING(order_id)
    WHERE (
        ol.status = 'Completed'
        OR (ol.status = 'Returned' AND EXISTS (
            SELECT 1 FROM item_level il2 WHERE il2.order_id = ol.order_id AND il2.returned = 0
        ))
    )
    AND ol.order_date >= date('now', '-90 days')
),
...
order_values_90d AS (
    ...
    FROM (
        SELECT ol.customer_id, ol.order_id,
            SUM(il.price_at_purchase * il.quantity) AS order_total,
            SUM(il.quantity) AS items_bought
        FROM order_level ol JOIN item_level il USING(order_id)
        WHERE ol.order_date >= date('now', '-90 days')
...
return_cancel_90d AS (
    ...
    FROM order_values_90d ov90 JOIN succ_order_values_90d sov90 USING(customer_id)
),
...
last_successful_order AS (
    SELECT t.customer_id, t.order_id,
        CAST(julianday('now') - julianday(t.order_date) AS INTEGER) AS last_successful_date
    FROM (
        SELECT ol.customer_id, ol.order_id, ol.order_date,
            ROW_NUMBER() OVER (PARTITION BY ol.customer_id ORDER BY ol.order_date DESC, ol.order_id DESC) AS rn
        FROM order_level ol
...
last_successful_stats AS (
    ...
    JOIN order_level ol ON ol.customer_id = lso.customer_id AND ol.order_id = lso.order_id
    JOIN item_level il ON il.order_id = lso.order_id
    GROUP BY lso.customer_id
),

full_return_orders AS (
    SELECT ol.order_id, ol.customer_id, ol.status_date,
...
last_return_order AS (
    ...
    FROM (
        ...
        JOIN last_return_date lrd ON fro.customer_id = lrd.customer_id AND fro.order_id = lrd.order_id
    ) t
),

last_return_stats AS (
    SELECT lro.customer_id,
    ...
    FROM order_level ol
    JOIN last_return_order lro ON lro.customer_id = ol.customer_id AND lro.order_id = ol.order_id
    JOIN item_level il ON il.order_id = lro.order_id
    GROUP BY lro.customer_id
),

last_cancel_order AS (
    SELECT t.customer_id, t.order_id,
        CAST(julianday('now') - julianday(t.status_date) AS INTEGER) AS days_from_last_cancel
    FROM (
        SELECT customer_id, order_id, status_date,
            ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY status_date DESC, order_id DESC) AS rn
        FROM order_level WHERE status = 'Cancelled'
    ) t WHERE rn = 1
),

last_cancel_stats AS (
    SELECT lco.customer_id,
    ...
    FROM order_level ol
    JOIN last_cancel_order lco ON lco.customer_id = ol.customer_id AND lco.order_id = ol.order_id
    JOIN item_level il ON il.order_id = lco.order_id
...
last3_succ_orders AS (
    SELECT * FROM (
        SELECT ol.customer_id, ol.order_id, ol.order_date, ol.status,
            ROW_NUMBER() OVER (PARTITION BY ol.customer_id ORDER BY ol.order_date DESC, ol.order_id DESC) AS rn
        FROM order_level ol
        WHERE ol.status = 'Completed'
           OR (ol.status = 'Returned' AND EXISTS (
               SELECT 1 FROM item_level il WHERE il.order_id = ol.order_id AND il.returned = 0
           ))
    ) t WHERE rn <= 3
),

last3_succ_orders_help_table AS (
    SELECT l3so.customer_id, l3so.order_id,
...
last3_orders_help_table AS (
    SELECT l3o.customer_id,
        SUM(CASE WHEN (l3o.status = 'Returned' AND il.returned = 1) OR l3o.status = 'Cancelled'
            THEN il.quantity * il.price_at_purchase ELSE 0 END) AS return_cancel_order_total,
        SUM(il.quantity * il.price_at_purchase) AS last_3orders_total,
        SUM(CASE WHEN (l3o.status = 'Returned' AND il.returned = 1) OR l3o.status = 'Cancelled'
            THEN il.quantity ELSE 0 END) AS return_cancel_order_items,
        SUM(il.quantity) AS last_3orders_items
    FROM last3_orders l3o JOIN item_level il USING(order_id)
...
last_session AS (
    SELECT customer_id, MAX(session_date) AS session_date
    FROM sessions_filtered
    WHERE session_date BETWEEN date('now', '-180 days') AND date('now')
    GROUP BY customer_id
),

last_session_stats AS (
    SELECT ls.customer_id,
        CAST(julianday('now') - julianday(ls.session_date) AS INTEGER) AS days_from_last_session,
        sf.pages_viewed AS last_session_pages_viewed
    FROM last_session ls
    JOIN sessions_filtered sf ON sf.customer_id = ls.customer_id AND sf.session_date = ls.session_date
),

successful_orders_180 AS (
    SELECT customer_id, COUNT(*) AS successful_orders_180
    FROM order_level ol
    WHERE ol.order_date BETWEEN date('now', '-180 days') AND date('now')
      AND (
          ol.status = 'Completed'
          OR (ol.status = 'Returned' AND EXISTS (
              SELECT 1 FROM item_level il2 WHERE il2.order_id = ol.order_id AND il2.returned = 0
...
sessions_180_stats AS (
    ...
    WHERE sfl.session_date BETWEEN date('now', '-180 days') AND date('now')
    GROUP BY sfl.customer_id
),

sessions_to_succ_orders_ratio_180 AS (
    SELECT s180.customer_id,
...
recent_sessions_90 AS (
    ...
    FROM sessions_filtered
    WHERE session_date BETWEEN date('now', '-90 days') AND date('now')
    GROUP BY customer_id
),
