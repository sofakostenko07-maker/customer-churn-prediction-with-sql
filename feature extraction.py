...
intervals AS (
    SELECT
        customer_id,
        DATEDIFF(order_date, LAG(order_date) OVER (PARTITION BY customer_id ORDER BY order_date)) AS interval_days
    FROM order_level
),
...
return_cancel_ratio AS (
    SELECT
        ov.customer_id,
        (rc.returned_items_total + rc.cancelled_items_total) / ov.total_orders_value AS return_cancel_ratio_lifetime
